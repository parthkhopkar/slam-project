...
class ParticleFilter:
    ...
    def predict(self, control):
        ...
        sigmaL = sqrt((alpha_1 * l) ** 2 + (alpha_2 * (l - r)) ** 2)
        sigmaR = sqrt((alpha_1 * r) ** 2 + (alpha_2 * (l - r)) ** 2)

        p_control = [(random.gauss(l, sigmaL), (random.gauss(r, sigmaR))) for p in self.particles]

        self.particles = [self.g(p, u, self.robot_width) for p, u in zip(self.particles, p_control)]
    ...
    def probability_of_measurement(self, measurement, predicted_measurement):
        ...
        d_meas, alpha_meas = measurement
        d_pred, alpha_pred = predicted_measurement

        delta_d = d_meas - d_pred
        delta_alpha = (alpha_meas - alpha_pred + pi) % (2*pi) - pi
        p_d = normal_dist.pdf(delta_d,0,self.measurement_distance_stddev)
        p_alpha = normal_dist.pdf(delta_alpha,0,self.measurement_angle_stddev)
    ...
    def compute_weights(self, cylinders, landmarks):
        ...
        for p in self.particles:
            ...
            # measurement = [l[0] for l in assignment]
            # assigned_landmarks = [l[1] for l in assignment]
            ...
            weight = 1.0
            for measurement, assigned_landmarks in assignment:
                predicted_measurement = self.h(p, assigned_landmarks, self.scanner_displacement)
